...
import { KpiChartRequestedGroupInfo } from './groups/kpi-chart-group-info.model';
...
export class KpiChart {
  ...
  public updateChartInfo(requestedGroupInfos: KpiChartRequestedGroupInfo[]): void {
    ...
    this.chartData.update(requestedGroupInfos);
  ...
  private drawChart(): void {
    ...
    this.band?.apply(this.bandSelection, this.config, this.chartData, this.dimensions, this.axis.scales);
  ...
  private initCommonSelections(container: HTMLDivElement): void {
    ...
    this.pointReferenceContainer = this.svgSelection.append('g').attr('class', 'chart-points').style('font', '14px MuseoSans300, sans-serif');
  }
  ...
  private svgToCanvas() {
    ...
    const svgString = this.domNodeToString(svg);
