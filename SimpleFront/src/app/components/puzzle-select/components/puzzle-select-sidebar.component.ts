...
@Component({
  selector: 'puzzle-select-sidebar',
  templateUrl: './puzzle-select-sidebar.component.html',
  styleUrls: ['./puzzle-select-sidebar.component.scss'],
  providers: [{
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => PuzzleSelectSidebarComponent),
      multi: true
  }],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class PuzzleSelectSidebarComponent<T extends ISortable> implements OnChanges, OnDestroy {
  @Input() available: T[];
  ...
  ngOnChanges(change: SimpleChanges): void {
    ...
    this.total = this.available.find( elem => elem[this.parentIdField] === null );
  }
  ...
  private setMaxHeightAndWidth(): void {
    ...
    this.maxDropdownWidth = 0.9 * winWidth;
    this.maxDropdownHeight = 0.9 * winHeight;
  }
